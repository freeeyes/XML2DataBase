...
{
	static ConnPool g_ConnPool;
	conn_pool = &g_ConnPool;
	return conn_pool;
}

void ConnPool::InitConnPool(const char *hostip, 
			const char *username, 
			const char *password,
			int maxSize, 
			int initSize){
	MutexGuard lock(mutex);
	MysqlEncap *conn = NULL;
	hostip_   = hostip;
	username_ = username;
	password_ = password;
	maxSize_  = maxSize;
...
MysqlEncap* ConnPool::GetOneConn()
{
	...
	else if(connList.size() > 0)
	{//注意区分connlist里面的是可用连接，而cursize表示所有建立的连接
		conn = connList.front();
		connList.pop_front();
	}
	else
